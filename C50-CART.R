...
req_packages <- c("survival", "funModeling", "tidyverse", "Hmisc", "LogicReg", 
                  "clusteval", "corrplot", "DMwR", "C50", "rpart")
...
weight_vec <- 0.1*(sapply(my_data[, 1], as.numeric) - 1) + rep(1, nrow(my_data))
## -------------------------------------------------------------------------- ##
# METHOD IMPLEMENTATION
CARTmodel <- rpart(Result ~., my_data, weights = weight_vec)
summary(CARTmodel)
my_data[, -1] <- as.data.frame(sapply(my_data[, -1], as.numeric))
C50Model <- C5.0(Result ~., my_data, 
                 weights = weight_vec, trial = 1)
summary(C50Model)
## -------------------------------------------------------------------------- ##
# ERRORS & RESULTS
par(xpd = T)
plot(CARTmodel)
text(CARTmodel, all = T)
plot(C50Model, subtree = NULL)

bin_data <- as.data.frame(sapply(all_data[, -1], as.logical))
pred_CART <- predict(CARTmodel, newdata = bin_data, type = "class")
pred_CART_bin <- sapply(pred_CART, as.numeric) - 1
table(all_data[, 1], pred_CART_bin)

bin_data <- as.data.frame(sapply(all_data[, -1], as.numeric))
pred_C50 <- predict(C50Model, newdata = bin_data)
pred_C50_bin <- sapply(pred_C50, as.numeric) - 1
table(all_data[, 1], pred_C50_bin)